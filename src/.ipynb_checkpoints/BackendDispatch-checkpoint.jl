...
using ..LNMock: DEFAULT_BACKEND

export FFTPlanSpec, make_plan, forwardFT!, inverseFT!,
       FourierArrayInfo, allocate_arrays

struct FFTPlanSpec
    dims::NTuple{3,Int}
    boxsize::NTuple{3,Real}
    volume::Real
    comm::Union{Nothing,Any}
    backend::Symbol
end

function FFTPlanSpec(dims::NTuple{3,Int}, boxsize::NTuple{3,Float64};
                     comm=nothing, backend=DEFAULT_BACKEND[])
    return FFTPlanSpec(dims, boxsize, prod(boxsize), comm, backend)
end


function forwardFT!(args...)
    if DEFAULT_BACKEND[] == :fftw_single
        return FFTW_Single.forwardFT!(args...)
    elseif DEFAULT_BACKEND[] == :fftw_threads
        return FFTW_Threads.forwardFT!(args...)
    elseif DEFAULT_BACKEND[] == :pencil_mpi
        return PencilMPI.forwardFT!(args...)
    end
end


function inverseFT!(args...)
    if DEFAULT_BACKEND[] == :fftw_single
        return FFTW_Single.inverseFT!(args...)
    elseif DEFAULT_BACKEND[] == :fftw_threads
        return FFTW_Threads.inverseFT!(args...)
    elseif DEFAULT_BACKEND[] == :pencil_mpi
        return PencilMPI.inverseFT!(args...)
    end
end
...
function make_plan(spec::FFTPlanSpec)
    ...
    elseif spec.backend == :pencil_mpi
        return PencilMPI.make_plan(spec)
    else
...
"""
function allocate_arrays(spec::FFTPlanSpec;fftplan=nothing)
    n1, n2, n3 = spec.dims

    if spec.backend == :pencil_mpi
       @assert fftplan !== nothing "PencilFFT backend requires a valid plan"
        # PencilFFTs handles local array shapes internally
        field_real    = allocate_input(fftplan)
        field_fourier = allocate_output(fftplan)
    else
