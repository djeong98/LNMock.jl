...
using FFTW, Random
using FLoops
using ..LNMock: DEFAULT_BACKEND, DEFAULT_PLANCK, compute_aHf, make_plan, forwardFT!, inverseFT!
...
function _Lognormal_galaxy(fpkgA,Nptls,Finfo,Nseed,xyzseed,gtempfname,deltak,deltar,v1,fftplan,comm=nothing)
    ...
    Nptls_result = gen_LN_galaxy_mock(gtempfname,nbar_target,deltar,Finfo.L1,Finfo.L2,Finfo.L3,Finfo.n1,Finfo.n2,Finfo.n3,Nseed,xyzseed)

    if !isnothing(comm)
        Nptls_total = MPI.Reduce(Nptls_result, +, 0, comm)
    else
...
function _Lognormal_add_velocity_and_save(fpkA,Nptls,Finfo,galfname,outfbase,deltak,deltar,v1,fftplan,aHf,nsubX,nsubY,nsubZ,iLN,comm=nothing)
    ...
    _save_galaxy_velocity(outfbase,galdata,Finfo.L1,Finfo.L2,Finfo.L3,nsubX,nsubY,nsubZ,iLN,comm=nothing)
...
"""
    ...
    Parameters:
    - merge_base: filebase*"_$iLN" (e.g., "output_5")
...
function run_lognormal(cfg::LNConfig)
    ...
    rank = cfg.comm === nothing ? 0 : MPI.Comm_rank(cfg.comm)
    if cfg.comm!=nothing
        println("Total number of processes = $(MPI.Comm_size(comm)), and my rank is $rank.")    
    end
    ...
    for iLN in 1:cfg.nreal
        ...
        gtempfname_this_rank = filebase*"_galtmp_$iLN_$rank.bin"
        Nptl_this = _Lognormal_galaxy(fpkgA,cfg.Ngals,Finfo,Nseed,xyzseed,gtempfname_this_rank,deltak,deltar,v1,fftplan,cfg.comm)

        # Of course, we have to use the same seed for the galaxy density and velocity field, to make them in phase.
        outbase = isnothing(cfg.comm) ? filebase*"_$iLN" : filebase*"_$iLN_$rank"
        
        success = _Lognormal_add_velocity_and_save(fpkA,Nptl_this,Finfo,gtempfname_this_rank,outfbase,deltak,deltar,v1,fftplan,aHfzred,cfg.nsub[1],cfg.nsub[2],cfg.nsub[3],iLN,cfg.comm)

        # remove the temp file
        if success
            rm(gtempfname_this_rank)
        end
        ...
        if !isnothing(cfg.comm)
            ...
            if rank==0
                nranks = MPI.Comm_size(cfg.comm)  # or set manually if MPI not defined
